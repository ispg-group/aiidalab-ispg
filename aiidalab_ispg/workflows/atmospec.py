...
from aiida.orm import StructureData, TrajectoryData, SinglefileData, ArrayData
from aiida.orm import Int, Float, Bool, Code, List, Dict
from aiida.orm import to_aiida_type

from aiidalab_ispg.wigner import Wigner
from .optimization import ConformerOptimizationWorkChain, structures_to_trajectory
...
class AtmospecWorkChain(WorkChain):
    ...
    def optimize(self):
        inputs = self.exposed_inputs(
            ConformerOptimizationWorkChain, namespace="opt", agglomerate=False
        )
        inputs.structure = self.inputs.structure
        inputs.orca.code = self.inputs.code
        return ToContext(
            optimization=self.submit(ConformerOptimizationWorkChain, **inputs)
        )
    ...
    def inspect_optimization(self):
        ...
        if not self.ctx.optimization.is_finished_ok:
            ...
            # TODO: Propagate the error from the child workchain
            return self.exit_codes.CONFORMER_ERROR
        self.out("relaxed_structures", self.ctx.optimization.outputs.relaxed_structures)
